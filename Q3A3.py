...
def main():
        ...
        for i in range(50):
            ...
            ra = rows[0]
            q = '''SELECT COUNT(DISTINCT(n.customer_id)) AS num, CAST(COUNT(c.order_id)/COUNT(DISTINCT(
            n.customer_id)) *1.00 AS FLOAT) AS average FROM Customers m,Orders n , Order_items c WHERE n.customer_id 
            = m.customer_id AND m.customer_postal_code= :random_postal_code AND n.order_id = c.order_id AND 
            n.order_id = c.order_id; '''
        ...
        connection.close()
        # Self-optimization - changes to constraints
        connection = sqlite3.connect(db_path)
        cursor = connection.cursor()
        ...
        bar_chart((sum(small_uninformed_runtime)) * n / 50, (sum(small_self_runtime)) * n / 50,
                  ...
                  (sum(lar_user_runtime)) * n / 50, "Running time for Query 3", "Different Databases",
