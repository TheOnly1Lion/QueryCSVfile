...
def main():
    ...
    for i in range(50):
        ...
        cursor.execute("SELECT order_id FROM Order_items Order by random() limit 1")
        rows = cursor.fetchone()
        ra = rows[0]
        q = '''SELECT count(DISTINCT s.seller_postal_code)
                FROM Sellers s, Order_items
                WHERE Order_items.seller_id = s.seller_id AND Order_items.order_id = :O '''
    ...
    bar_chart((sum(small_uninformed_runtime)) * n / 50, (sum(small_self_runtime)) * n / 50,
              ...
              (sum(lar_user_runtime)) * n / 50, "Running time for Query 4", "Different Databases",
