...
def main():
    ...
    # Setting scenario “Uninformed” for small database
    cursor.execute("DROP VIEW IF EXISTS order_size")
    cursor.execute("CREATE VIEW order_size AS SELECT order_id,COUNT(*) AS size FROM order_items GROUP BY order_id")
    ...
    for i in range(50):
        ...
        ra = rows[0]
        q = '''SELECT order_id, AVG(size) FROM order_size WHERE order_id IN(SELECT order_id FROM customers cust, 
        orders ord WHERE cust.customer_id = ord.customer_id AND customer_postal_code =:o); '''
        cursor.execute(q, {'o': ra})
    ...
    connection.close()
    # Self-optimization - changes to constraints for small database
    connection = sqlite3.connect(db_path)
    cursor = connection.cursor()
    cursor.execute("DROP VIEW IF EXISTS order_size")
    cursor.execute("CREATE VIEW order_size AS SELECT order_id,COUNT(*) AS size FROM order_items GROUP BY order_id")
    ...
    # connecting to medium database
    db_path = './A3Medium.db'
    connection = sqlite3.connect(db_path)
    cursor = connection.cursor()
    # Setting scenario “Uninformed” for medium database
    cursor.execute("DROP VIEW IF EXISTS order_size")
    cursor.execute("CREATE VIEW order_size AS SELECT order_id,COUNT(*) AS size FROM order_items GROUP BY order_id")
    cursor.execute(' PRAGMA automatic_index=False;')
    cursor.execute(' PRAGMA foreign_keys=OFF; ')
    ...
    bar_chart((sum(small_uninformed_runtime)) * n / 50, (sum(small_self_runtime)) * n / 50,
              ...
              (sum(lar_user_runtime)) * n / 50, "Running time for Query 2", "Different Databases",
